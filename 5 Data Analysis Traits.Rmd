...
```{r}
lm_sla_b_alt <- lm(log(erm+1-min(data_species_b$erm)) ~ mean_SLA, data = data_species_b)
plot(lm_sla_b_alt)
check_normality(lm_sla_b_alt)
...
```{r}
lm_sla_c_alt <- lm(erm ~ log10(mean_SLA), data = data_species_c)
lm_sla_b_alt <- lm(erm ~ log10(mean_SLA), data = data_species_b)
plot(lm_sla_c_alt)
plot(lm_sla_b_alt)
...
```{r}
graph_sla_c <- ggplot(data_species_c)+
  geom_point(aes(x = mean_SLA, y = erm), colour = mypal[5]) +
  theme_pubr() +
  scale_x_continuous(name = "Specific Leaf Area (cm^2/g)",
                     limits = c(0, 130),
                     breaks = c(0, 20, 40, 60, 80, 100, 120)) +
...
##HAIRS
## Does enemy release differ in species with different types of defence?
...
graph_hair_b <- data_species_b %>% filter(!is.na(hairs)) %>% ggplot() +
  ...
  scale_x_discrete(name = "",
                   labels = c("Glabrous leaves", "Hairy leaves"))
...
```{r}
lm_dispersal_b <- lm(erm ~ dispersal_syndrome1, data = data_species_b)
...
```{r}
lm_dispersal_c <- lm(erm ~ dispersal_syndrome1, data = data_species_c)
...
```{r}
ann_per <- annotate_figure(fig.lab = "4)", fig.lab.face = "bold", fig.lab.size = 14,
                                 ggarrange(ncol = 2, nrow = 1,
                                           graph_dispersal_b,
                                           graph_ann_c + rremove("ylab"),
...
```{r}
clonal <- annotate_figure(fig.lab = "6)", fig.lab.face = "bold", fig.lab.size = 14,
                                 ggarrange(ncol = 2, nrow = 1,
                                           graph_clonal_b,
                                           graph_clonal_c + rremove("ylab"),
                                           labels = c("Biogeographic", "Community"), 
                                           hjust = -1.2))
...
```{r}
ggexport(clonal , res = 300, width = 2244*1, height = 2244*0.5, filename = "./output/ch4 figures/clonal.tiff")
```